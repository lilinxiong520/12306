...
if (typeof (station_names) != "undefined") {
	...
	for ( var i = 0; i < cities.length; i++) {
		...
		if (raha == "a" || raha == "b" || raha == "c" || raha == "d"
				|| raha == "e") {
...
function tHtmlGetCityName(nod, at, aPageNo) {
	...
	case 0:
		...
		// if(pagecount > 1){
		// ularray_cities_showing0 = fav_cities.slice(ulPageSize*(aPageNo),
		// Math.min(ulPageSize*(aPageNo+1), fav_cities.length) );
...
function showAllCity() {
	...
	// $("#form_cities2").css("display","none");
	var tHtml = "";
	// tHtml = '<div style="position: absolute; z-index: 2000; top: 140px; left:
	// 138.5px;" winstyle="hot">'
	...
	for ( var a = 2; a <= 6; a++) {
		...
		if (totelLength > ulPageSize) {
			...
			var pagecount = Math.ceil((totelLength + 1) / ulPageSize);
			if (pagecount > 1) {
				tHtml += '<ul  class="popcitylist" style="overflow: auto; max-height: 260px; height: 170px;display:none;" id="ul_list'
						+ a + '">';
				pageDesigh(pagecount, 0, a);
			}
			// for(var b=0;b<ularray_cities_showing.length;b++){
			// var show = ularray_cities_showing[b];
			// tHtml += '<li class="ac_even" title="'+show[1]+'"
...
function pageDesigh(pagecount, aPageNo, idIndex) {
	...
	if (pagecount > 1) {
		...
		for ( var b = 0; b < ularray_cities_showing.length; b++) {
			var show = ularray_cities_showing[b];
			ulHtml += '<li class="ac_even"   title="' + show[1] + '" data="'
					+ show[2] + '">' + show[1] + '</li>';
		}
...
// function isMatchCity(aCityInfo, aKey, aisPinyin){
// var aKey = aKey.toLowerCase();
// var aInfo = [aCityInfo[0].toLowerCase(),
// aCityInfo[1],aCityInfo[4].toLowerCase()];
...
// for(var m=0; m< aKey.length; m++){
// var newIndex = aInfo[0].indexOf( aKey[m]);
// var newIndexPy = aInfo[2].indexOf( aKey[m]);
// // alert(aInfo[0]);
// if(newIndex>lastIndex && newIndex<=m){
// aInfo[0]= replaceChar(aInfo[0], newIndex, "-");
// lastIndex= newIndex;
// }else if(newIndexPy>lastIndex && newIndexPy<=m){
// aInfo[2]= replaceChar(aInfo[2], newIndexPy, "-");
// lastIndex= newIndexPy;
// }else{
// return false;
// }
// }
			...
			$("#fromStationText")
					.keydown(
							...
							function() {
								...
								if (isClick) {
									...
									if (cur == -1) {
										...
										$('.popcitylist')
												.css("display", "none");
										$("#ul_list1").css("display", "block");
										$("#flip_cities2").css("display",
												"none");
										$("#nav_list1").addClass("action");
										$('.ac_even').removeClass('ac_over')
												.addClass('ac_odd');

										$("#form_cities2").css("display",
												"block");
									}
								...
								curObjCode = $("#fromStation");
								cur = 0;
								var fromStationText = $("#fromStationText")
										.val();
