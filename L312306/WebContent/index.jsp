...
<script type="text/javascript">
	...
	/* $(document).ready(function(){
		$("#buyticket").click(function(){
			...
			var fromStation = "${routeInfos[0].fromStation}";
			var toStation = "${routeInfos[0].toStation}";
		  $.ajax({
             success:function(data){
   	           	 location.href = 
	           		 "${pageContext.request.contextPath}/route_getTrain.action?tid="+tid+"&fromStation="+
...
<body >
	...
	<form action="route_find_from_to_station.action" method="post">
		...
		<input name="toStation" maxlength="15" value="${routeInfos[0].toStation}" placeholder="请输入终点站" id="toStationText" style="width:100px;" class="input_20txt_gray" type="text" />
		 出发日： <input type="text" id="spandate"
			class="input_20txt_gray" id="train_start_date" maxlength="15">&nbsp;&nbsp;
		<input id="query" shape="rect" class="btn122s" value="查询" type="submit" style="color:#ffffff " />
			&nbsp;&nbsp; 
			...
			<tbody id="tbody" style="text-align: center;">
				<c:forEach var="route" items="${routeInfos}">
					<tr>
						...
						<td>--</td>   <!--  <th>其它</th>  -->               
						<td>
							<a href="${pageContext.request.contextPath}/route_getTrain.action?tid=${route.tid}&fromStation=${route.fromStation}&toStation=${route.toStation}" >
								购买
							</a>
						</td>
						<td>--</td>		<!-- <th>备注</th> --> 
